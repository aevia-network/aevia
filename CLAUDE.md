...
This repo adopts the GSD (Get Shit Done) pipeline. State of record lives under `.planning/`.

**Read first, always:**

1. `.planning/STATE.md` — digest: current phase, plan, last activity, blockers. Pivot for every session.
2. `.planning/PROJECT.md` — premises: thesis, core value, validated/active requirements, key decisions, constraints.
3. `.planning/ROADMAP.md` — phase structure, dependencies, success criteria.
4. `.planning/REQUIREMENTS.md` — REQ-IDs mapped to phases.
5. `.planning/codebase/` — brownfield map generated by `/gsd-map-codebase`.

**Do not edit these four docs directly.** They mutate only via workflow commands:

- `/gsd-discuss-phase N` — gather context for a phase before planning
- `/gsd-plan-phase N` — produce `PLAN.md` inside `.planning/phase-N-*/`
- `/gsd-execute-phase N` — execute plans with atomic commits
- `/gsd-verify-work` / `/gsd-transition` — close a phase and update STATE.md + PROJECT.md

**Pipeline precedence:** when a phase is active (STATE.md shows non-zero progress or an active plan), prefer `/gsd-*` commands over ad-hoc edits to code implementing phase requirements. Ad-hoc work outside the current phase's scope belongs in `/gsd-add-backlog` or `/gsd-insert-phase` — never drop it silently into the active phase.

**Worktrees:** each phase can run in its own `/Users/leandrobarbosa/Personal/videoengine-<slug>` worktree (config.json has `workflow.use_worktrees: true`). Destroy the worktree after merge back to `main`.
